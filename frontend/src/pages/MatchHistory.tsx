...
import { fetchLastFiveMatches, fetchOverallMatchHistory } from "../util/API";
  ...
  const MatchHistory: React.FC = () => {
    ...
    const [overallMatchHistory, setOverallMatchHistory] = useState<OverallMatchHistory | null>(null);
    ...
    const getOverallMatchHistory = async () => {
        console.log('ran getOverallMatchHistory');
        console.log(teamOne);
        console.log(teamTwo);
        if (teamOne && teamTwo) {
            console.log('Both non null')
            const res = await fetchOverallMatchHistory(teamOne.team_id, teamTwo.team_id)
            console.log(res)
        ...
        <Center minHeight="100vh" minWidth="100%" bg="gray.200">
          ...
          >
            ...
            <ModalContent>
                ...
                {(switcher ? teamResults : emptyList)?.filter((item) =>
            ...
            <Table variant="simple">
              <Thead>
                <Tr>
                  <Th>{teamOne?.team_long_name ? teamOne?.team_long_name + " wins" : "Team 1 Wins"}</Th>
                  <Th>{teamTwo?.team_long_name ? teamTwo?.team_long_name + " wins" : "Team 2 Wins"}</Th>
                  <Th>Ties</Th>
                </Tr>
              </Thead>
              <Tbody>
            ...
            <Table variant="simple">
              <Thead>
                <Tr>
                    <Th>Match Season</Th>
                    <Th>{teamOne?.team_long_name ? teamOne?.team_long_name + " Score" : "Team 1 Score"}</Th>
                    <Th>{teamTwo?.team_long_name ? teamTwo?.team_long_name + " Score" : "Team 2 Score"}</Th>
                </Tr>
              </Thead>
              <Tbody>
